...
const makeChainable = (table: string) => {
  ...
  const chain: Record<string, unknown> = {
    ...
    then: (fn: (value: typeof result) => unknown) => Promise.resolve(result).then(fn),
    catch: (fn: (reason: unknown) => unknown) => Promise.resolve(result).catch(fn),
    eq: () => chain,
    neq: () => chain,
    gt: () => chain,
    lt: () => chain,
...
vi.mock('../../data/videoStore', () => ({
  videoStore: {
    ...
    subscribe: vi.fn((cb: (data: unknown[]) => void) => { setTimeout(() => cb?.(mockSupabaseData.videos), 0); return vi.fn(); }),
    saveVideos: vi.fn(),
  },
  Video: {},
}));
...
vi.mock('../../utils/adminApi', () => ({
  adminFetch: (...args: unknown[]) => mockAdminFetch(...(args as [string, Record<string, unknown>])),
...
vi.mock('framer-motion', () => ({
  motion: new Proxy({}, {
    get: (_t, prop) => typeof prop === 'string'
      ? React.forwardRef(({ children, ...p }: Record<string, unknown> & { children?: React.ReactNode }, ref: React.Ref<Element>) => {
