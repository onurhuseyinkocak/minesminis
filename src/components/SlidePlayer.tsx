    ...
    <>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Link href="/slides" className="mm-icon-btn" aria-label="Sunumlara dön"><ArrowLeft size={18} /></Link>
          <div>
        ...
        <>
          <div style={{ background: 'white', borderRadius: 28, overflow: 'hidden', border: '1px solid var(--line)', boxShadow: 'var(--shadow-1)' }}>
            <div style={{ aspectRatio: '16/9', position: 'relative', overflow: 'hidden' }}>
              ...
              {currentItem && (
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ background: 'rgba(255,255,255,0.92)', padding: '20px 32px', borderRadius: 20, textAlign: 'center' }}>
                    <div style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 700, letterSpacing: 2 }}>#{current + 1}</div>
                    <div style={{ fontFamily: 'var(--font-display)', fontSize: 52, fontWeight: 800, color: 'var(--primary)', letterSpacing: -1, lineHeight: 1.1 }}>
                      {currentItem.label}
                    </div>
              ...
              {current > 0 && (
                <button onClick={goPrev} aria-label="Önceki" style={{ ...arrowBtnStyle, left: 12 }}>
                  <ChevronLeft size={24} />
                </button>
              )}

              {current < total - 1 && (
                <button onClick={goNext} aria-label="Sonraki" style={{ ...arrowBtnStyle, right: 12 }}>
            ...
            <div style={{ padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 12, borderTop: '1px solid var(--line)' }}>
              <button className="mm-icon-btn primary" style={{ width: 44, height: 44 }} onClick={() => setPlaying(!playing)} aria-label={playing ? 'Durdur' : 'Otomatik oynat'}>
