    ...
    return (
      <Layout>
        <div style={{ textAlign: 'center', padding: 60 }}>
          ...
          <Link to="/slides" className="mm-btn primary" style={{ marginTop: 12, textDecoration: 'none', display: 'inline-flex' }}>Slaytlara don</Link>
        </div>
      </Layout>
    )
  }
    ...
    <Layout>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Link to="/slides" className="mm-icon-btn" aria-label="Slaytlara don"><ArrowLeft size={18} /></Link>
          <div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700 }}>{slide.title}</div>
            <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>Slayt {current + 1} / {total}</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="mm-btn dark" onClick={() => document.documentElement.requestFullscreen?.()} aria-label="Tam ekran">
      ...
      {items.length > 0 ? (
        <>
          <div style={{ background: 'white', borderRadius: 28, overflow: 'hidden', border: '1px solid var(--line)', boxShadow: 'var(--shadow-1)' }}>
            ...
            <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, borderTop: '1px solid var(--line)' }}>
              <button className="mm-icon-btn" onClick={() => setCurrent(c => Math.max(0, c - 1))} aria-label="Onceki slayt"><SkipBack size={18} /></button>
              <button className="mm-icon-btn primary" style={{ width: 52, height: 52 }} onClick={() => setPlaying(!playing)} aria-label={playing ? 'Durdur' : 'Otomatik oynat'}>
                {playing ? <Pause size={20} /> : <Play size={20} />}
              </button>
              <button className="mm-icon-btn" onClick={() => setCurrent(c => Math.min(total - 1, c + 1))} aria-label="Sonraki slayt"><SkipForward size={18} /></button>
              <div style={{ flex: 1 }}>
        ...
        <div style={{ textAlign: 'center', padding: 60, color: 'var(--ink-3)', background: 'white', borderRadius: 28, border: '1px solid var(--line)' }}>
          <p style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700 }}>Slayt icerigi henuz eklenmedi</p>
        </div>
      )}
    </Layout>
  )
