...
export default function NotFound() {
  useEffect(() => {
    document.title = '404 — Sayfa Bulunamadı | minesminis'

    // Mark as noindex so search engines / AdSense don't treat this as a content page
    ...
    return () => {
      // Restore: remove noindex on unmount
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', minHeight: '50vh' }}>
      <div style={{
        ...
        borderRadius: 28, padding: '48px 28px', textAlign: 'center', marginBottom: 24,
      }}>
        ...
        <Link to="/" className="mm-btn primary" style={{ textDecoration: 'none' }}>
          <Home size={16} /> Anasayfa
        </Link>
      </div>

      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 20, color: 'var(--ink)', marginBottom: 12 }}>
        Hızlı erişim
      </h2>
      <div style={{
      ...
      <div style={{
        ...
        lineHeight: 1.7, fontSize: 14.5, color: 'var(--ink-2)',
      }}>
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 17, margin: '0 0 8px', color: 'var(--ink)' }}>
          Yine de bir hata gibi mi geliyor?
        </h3>
        <p style={{ margin: 0 }}>
          Bir bağlantıdan geldiyseniz ve sayfanın gerçekten var olması gerekiyorsa, lütfen <Link to="/contact" style={{ color: 'var(--accent)', fontWeight: 600 }}>iletişim</Link> sayfasından hangi URL'i ziyaret ettiğinizi bize yazın. 1-3 iş günü içinde dönüş yaparız.
        </p>
      </div>
    </div>
  )
