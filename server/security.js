...
export function rateLimiter(options = {}) {
    const { maxRequests = 100, windowMs = 60000, keyGenerator, keyPrefix } = options;

    return (req, res, next) => {
        const base = keyGenerator ? keyGenerator(req) : getClientIP(req);
        const key = keyPrefix ? `${keyPrefix}:${base}` : base;
        const now = Date.now();
...
export function securityHeaders(req, res, next) {
    ...
    res.setHeader('Content-Security-Policy', [
        "default-src 'self'",
        "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.googletagmanager.com https://apis.google.com https://*.firebaseapp.com",
        "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
        "font-src 'self' https://fonts.gstatic.com",
        "img-src 'self' data: https: blob:",
        "connect-src 'self' https://*.supabase.co https://api.openai.com wss://*.supabase.co http://localhost:3001 ws://localhost:3001 https://*.googleapis.com https://*.firebaseio.com",
